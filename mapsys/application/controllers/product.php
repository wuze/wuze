...
class Product extends CI_Controller {
	...
	public function index()
	{
		...
		$page_config['url']='/product/showlist';//url
		...
		$sql = " SELECT COUNT(1) as cnt FROM ddimages WHERE type='product' ";
		...
		$sql = " SELECT * FROM ddimages WHERE type='product' LIMIT ".$offset.",".$page_config['perpage'];
		$ret = $this->db->query( $sql );
		foreach( $ret->result_array() as  $row ){
			$views[] = $row;
		}
	...
	public function showimg(){
		...
		$data['hidden_id'] = $id;
		$data['page_title']="产品展示";
		$this->load->view('image_read',$data);
	}
